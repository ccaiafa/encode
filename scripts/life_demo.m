...
%  a few minutes requiring only about 2GB of computer RAM and standard 
%  hardaware. 
%
...
switch strcat(mycomputer,'_',release)
        ...
        fe_prob = feConnectomeInit(dwiFile,fgFileName,feFileName,[],dwiFileRepeat,t1File,N,[1,0],0);
        otherwise
        ...
        fe_prob = feConnectomeInit(dwiFile,fgFileName,feFileName,[],dwiFileRepeat,t1File,N,[1,0],1);
end
...
fe_prob = feSet(fe_prob,'fit',feFitModel(feGet(fe_prob,'model'),feGet(fe_prob,'diffusionsignalinvoxel'),feGet(fe_prob,'dsigdemeaned'),feGet(fe_prob,'s0_img'),'bbnnls',2000,'preconditioner'));
...
prob_demeaned.rmse   = feGet(fe_prob,'vox rmse');
prob_with_mean.rmse =  feGet(fe_prob,'vox rmse with mean');
...
prob_demeaned.rmsexv = feGetRep(fe_prob,'vox rmse');
prob_with_mean.rmsexv =  feGetRep(fe_prob,'vox rmse with mean');
...
prob_demeaned.rrmse  = feGetRep(fe_prob,'vox rmse ratio');
prob_with_mean.rrmse =  feGetRep(fe_prob,'vox rmse ratio with mean');
...
prob_demeaned.w      = feGet(fe_prob,'fiber weights');
prob_with_mean.w      = feGet(fe_prob,'fiber weights');

%% (1.7) Plot a histogram of the RMSE. 
% We plot the histogram of  RMSE across white-mater voxels.
[fh(1), ~, ~] = plotHistRMSE(prob_with_mean);
...
[fh(2), ~] = plotHistRrmse(prob_with_mean);

%% (1.9) Plot a histogram of the fitted fascicle weights. 
[fh(3), ~] = plotHistWeights(prob_with_mean);
...
p.coords = feGet(fe_prob,'roi coords');
...
switch strcat(mycomputer,'_',release)
        case {'GLNXA64_2015a','MACI64_2014b'}
        fe_det = feConnectomeInit(dwiFile,fgFileName,feFileName,[],dwiFileRepeat,t1File,N,[1,0],0);
        otherwise
        sprintf('WARNING: currently LiFE is optimized for an efficient usage of memory \n using the Sparse Tucker Decomposition aproach (Caiafa&Pestilli, 2015) \n ONLY for Linux (MatlabR2015a) and MacOS (MatlabR2014b). \n If you have a different system or version you can still \n use the old version of LiFE (memory intensive). \n\n')
        sprintf('\n Starting building big matrix M in OLD LiFE...\n')
        fe_det = feConnectomeInit(dwiFile,fgFileName,feFileName,[],dwiFileRepeat,t1File,N,[1,0],1);
end
...
% method. The information generated by fitting the model (fiber weights
% etc) is then installed in the LiFE-BD structure.
fe_det = feSet(fe_det,'fit',feFitModel(feGet(fe_det,'model'),feGet(fe_det,'diffusionsignalinvoxel'),feGet(fe_det,'dsigdemeaned'),feGet(fe_det,'s0_img'),'bbnnls',2000,'preconditioner'));
...
det_with_mean.w      = feGet(fe_det,'fiber weights');

%% (2.7) Plot a histogram of the RMSE. 
% We plot the histogram of  RMSE across white-mater voxels.
[fh(1), ~, ~] = plotHistRMSE(det_with_mean);

%% (2.8) Plot a histogram of the RMSE ratio.
% As a reminder the Rrmse is the ratio between data test-retest reliability
% and model error (the quality of the model fit).
[fh(2), ~] = plotHistRrmse(det_with_mean);

%% (2.9) Plot a histogram of the fitted fascicle weights. 
[fh(3), ~] = plotHistWeights(det_with_mean);

%% Extract the coordinates of the white-matter voxels.
% We will use this later to compare probabilistic and deterministic models.
d.coords = feGet( fe_det, 'roi coords');
...
prob_with_mean.coordsIdx = ismember(p.coords,d.coords,'rows');
prob_with_mean.coords    = p.coords(prob_with_mean.coordsIdx,:);
det_with_mean.coordsIdx  = ismember(d.coords,prob_with_mean.coords,'rows');
det_with_mean.coords     = d.coords(det_with_mean.coordsIdx,:);
prob_with_mean.rmse      = prob_with_mean.rmse( prob_with_mean.coordsIdx);
det_with_mean.rmse       = det_with_mean.rmse( det_with_mean.coordsIdx);
clear p d

%% (3.2) Make a scatter plot of the RMSE of the two tractography models
fh(4) = scatterPlotRMSE(det_with_mean,prob_with_mean);
...
se = feComputeEvidence(prob_with_mean.rmse,det_with_mean.rmse);
