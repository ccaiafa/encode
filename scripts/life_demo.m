...
fe = feSet(fe,'fit',feFitModel(feGet(fe,'model'),feGet(fe,'diffusionsignalinvoxel'),feGet(fe,'dsigdemeaned'),feGet(fe,'s0_img'),'bbnnls'));
...
prob_demeaned.rmse   = feGet(fe,'vox rmse');
prob_with_mean.rmse =  feGet(fe,'vox rmse with mean');
...
prob_demeaned.rmsexv = feGetRep(fe,'vox rmse');
prob_with_mean.rmsexv =  feGetRep(fe,'vox rmse with mean');
...
prob_demeaned.rrmse  = feGetRep(fe,'vox rmse ratio');
prob_with_mean.rrmse =  feGetRep(fe,'vox rmse ratio with mean');
...
prob_demeaned.w      = feGet(fe,'fiber weights');
prob_with_mean.w      = feGet(fe,'fiber weights');
...
% We now use the LiFE-BD structure and the fit to compute the error in each
% white-matter voxel spanned by the tractography model.
det_demean.rmse   = feGet(fe,'vox rmse');
det_with_mean.rmse   = feGet(fe,'vox rmse with mean');

%% (2.4) Extract the RMSE of the model on the second data set. 
% Here we show how to compute the cross-valdiated RMSE of the tractography
% model in each white-matter voxel. We store this information for later use
% and to save computer memory.
det_demean.rmsexv = feGetRep(fe,'vox rmse');
det_with_mean.rmsexv = feGetRep(fe,'vox rmse with mean');

%% (2.5) Extract the Rrmse. 
% We show how to extract the ratio between the model prediction error
% (RMSE) and the test-retest reliability of the data.
det_demean.rrmse  = feGetRep(fe,'vox rmse ratio');
det_with_mean.rrmse  = feGetRep(fe,'vox rmse ratio with mean');

%% (2.6) Extract the fitted weights for the fascicles. 
% The following line shows how to extract the weight assigned to each
% fascicle in the connectome.
det_demean.w      = feGet(fe,'fiber weights');
det_with_mean.w      = feGet(fe,'fiber weights with mean');
