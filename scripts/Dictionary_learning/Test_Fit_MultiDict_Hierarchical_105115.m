...
lambda = 0.5; % Parameter that control l2 regularization (maybe will be eliminated in the future)
...
while (~exit_cond)&& (iter<16)
    ...
    while (error_weights > 0.0001) || (delta_error > 1E-10)
        ...
        disp(['Iter=',num2str(iFit),' Fitting=',num2str(iFit),' Delta weights=',num2str(100*error_weights),'%',' error full=',num2str(100*error_full),'%']);
    ...
    fe_new = fe_DivDict(fe, iter+1, lambda ); % Dvide first dictionary
    fe_new =  feFitDictionaries(fe_new,lambda); % Fit new Dictionaries
    pred_full = feGet(fe,'predfull');
    meas_full = feGet(fe,'dsigmeasured');
    error_full =  norm(meas_full - pred_full,'fro')/norm(meas_full,'fro');
    ...
    save('fe_structure_MultiHier_105115.mat','fe','-v7.3');
